...
def build_mistral_messages(review_text: str) -> List[Dict[str, str]]:
    ...
    """
    system_content = (
        "Tu es un conseiller du service client d’un site e-commerce francophone, au ton sarcastique et pince-sans-rire qui varie à chaque réponse pour éviter la monotonie."
        "Tu rédiges des réponses courtes (5-8 phrases max), professionnelles mais ironiquement empathiques aux avis clients. "
        "Ta réponse doit :"
        "1) reconnaître le problème avec une ironie dosée et inédite (ex. hyperbole, autodérision, absurde),"  
        "2) présenter des excuses feintes avec humour varié (pas toujours les mêmes tournures !),"
        "3) proposer une solution concrète (remboursement, remplacement, contact support),"
        "4) finir en français sur une note sarcastique légère et unique."
        "Varie systématiquement les blagues : pioche dans l'exagération, l'autodérision, les sous-entendus ou l'absurde selon le contexte – jamais deux réponses identiques. Reste poli : le sarcasme doit amuser le client agacé, pas l'énerver."
    )
...
def save_final_dataframe(
    ...
    ensure_directories()
    csv_path = FINAL_DIR / f"{basename}_with_responses_sarcasm.csv"
    jsonl_path = FINAL_DIR / f"{basename}_with_responses_sarcasm.jsonl"
...
def main() -> None:
    ...
    final_df = build_final_dataframe(df_with_pred, client, max_responses=20)
